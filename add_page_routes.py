...
def _order_value(value, default=float('inf')):
    try:
        if value is None:
            return default
...
def _sort_sections(section_list):
    section_list = section_list or []
    return sorted(
        section_list,
        key=lambda item: (
            _order_value(item.get('section_order')),
            item.get('section_key') or '',
        ),
    )


def _sort_columns(column_list, section_order_map=None):
    column_list = column_list or []
    section_order_map = section_order_map or {}
    return sorted(
        column_list,
        key=lambda item: (
            _order_value(section_order_map.get(item.get('tab'))),
            _order_value(item.get('column_order')),
...
def follow_sop_route():
    ...
    # 섹션 정보 가져오기 (section_order ASC 보장)
    sections = _sort_sections(section_service.get_sections())
    section_order_map = {
        sec.get('section_key'): _order_value(sec.get('section_order'))
        for sec in sections
    }
    section_order_map = {
        sec.get('section_key'): _order_value(sec.get('section_order'))
    ...
    section_order_map = {sec.get('section_key'): idx for idx, sec in enumerate(sections)}
    section_order_map = {sec.get('section_key'): idx for idx, sec in enumerate(sections)}
    try:
    ...
    dynamic_columns = _sort_columns([dict(row) for row in dynamic_columns_rows], section_order_map)
    # 컬럼 타입 정규화 - 공통 함수 사용
...
def full_process_detail(fullprocess_number):
    ...
    cursor.execute("""
        ...
        ORDER BY column_order
    """)
    dynamic_columns = _sort_columns([dict(row) for row in cursor.fetchall()], section_order_map)
