                ...
                if isinstance(v, str):
                    ...
                    if s.startswith('[') and s.endswith(']'):
                        try:
                            ...
                            logger.error(f"[{column_key}] 배열 문자열 파싱 실패: {e}")
...
def save_dropdown_codes():
    ...
    try:
        ...
        logger.info(f"[dropdown-codes] v3 handler called: column_key={column_key}, codes count={len(codes)}")
        logger.info(f"[dropdown-codes] raw codes: {codes}")
        ...
        logger.info(f"[dropdown-codes] flattened to {len(flattened)} values: {[f['value'] for f in flattened]}")
